...
After you clone the plugin from the original repo (https://github.com/rihp/startgpt-planner-plugin) Add it to the plugins folder of your StartGPT repo and then run StartGPT
...
```
zip -ru ../fork/plugins/planner.zip . ; cd ../fork && python3 -m startgpt --debug 
```

then you need to cd back to 
```
cd ../startgpt-planner-plugin    
```
