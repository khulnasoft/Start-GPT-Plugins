...
class TestNewsSearch:
    def mock_response(self, *args, **kwargs):
        # Mock Response of NewsAPI. We have result for StartGPT in technology but not others,
        # whereas Cricket is present in Sports/Entertainment but not others
        if kwargs["q"] == "AI" and kwargs["category"] == "technology":
            return json.loads(
                """{"status":"ok","totalResults":1,"articles": [{"title": "StartGPT"}]}"""
            )
    ...
    def test_news_search(self):
        ...
        actual_output_startgpt = self.NewsSearch.news_headlines_search_wrapper("AI")
        assert actual_output_startgpt.count(["StartGPT"]) == 1
        assert actual_output_startgpt.count([]) == 5
